...
- `CLAUDE.md` - issue tracking and honesty rules added
- `issue_documentations/BUGS.md` - created, bug #1 closed, bug #2 open
- `issue_documentations/bug_references/` - created with 5 reference screenshots
...
1. **Fix bug #2 properly** - show all foods at top, recent foods ALWAYS visible below (even during search). Do not hide recent. Just show filtered results above and recent below at all times.

2. **Test bug #2 fix on iPhone** - the ordering changes have only been tested on desktop so far

3. **Add the session summary to the session-003 file** - session-003 was created mid-session on Feb 18 and may need updating with what was done later in that session

4. **Continue building features** - from session-003 next steps: delete food entries, visual design polish to match reference, scrollable nutrition details panel
